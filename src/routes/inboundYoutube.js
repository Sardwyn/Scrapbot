// routes/inboundYoutube.js
// Handles inbound YouTube events forwarded from Dashboard
...
router.post("/api/inbound/youtube", async (req, res) => {
    ...
    if (!isChatEvent) {
        if (typeof RoomIntelService.recordTelemetry === "function") {
            try {
                ...
                console.warn("[inboundYoutube] recordTelemetry failed", e?.message || e);
            }
        }
    ...
    if (modResult && (modResult.action === "block" || modResult.action === "ban")) {
        console.log(`[YouTube] Blocked message from ${envelope.author?.username}`);
    ...
    try {
        ...
        console.warn("[YouTube] RoomIntel ingest failed", err.message);
    }
